...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if(!host.empty() && host[0] == '[') {
        ...
        if(endipv6) {
            //TODO check out of range 越界情况：*(endipv6+1) == '\0'
            if(*(endipv6 + 1) == ':') {
                service = endipv6 + 2;
            }
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    hints.ai_flags = AI_NUMERICHOST;  // 只能是数字化的地址字符串
...
IPAddress::ptr IPv4Address::networdAddress(uint32_t prefix_len) {
    ...
    sockaddr_in baddr(m_addr);
    // 注释部分是另一种实现逻辑
    // const sockaddr_in * subnet = (const sockaddr_in *)(subnetMask(prefix_len)->getAddr());
    // baddr.sin_addr.s_addr &= subnet->sin_addr.s_addr;
...
std::ostream& IPv6Address::insert(std::ostream& os) const {
    os << "[";
    // IPv6是8个16位字段组成，每个字段都必须包含一个十六进制数字
...
UnixAddress::UnixAddress(const std::string& path) {
    ...
    // path[0] == '\0' 就代表empty是true，当然可能有我不知道的
    // 不认为会有下面的情况
...
std::string UnixAddress::getPath() const {
    std::stringstream ss;
    // sun_path = "" 的时候还有一个'\0'，所以依旧m_length > offsetof(sockaddr_un, sun_path)
    if(m_length > offsetof(sockaddr_un, sun_path)
            && m_addr.sun_path[0] == '\0') {
        // std::string(m_addr.sun_path + 1, 0) == 空
