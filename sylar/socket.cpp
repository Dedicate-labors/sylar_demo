...
Socket::ptr Socket::CreateUDP(sylar::Address::ptr address) {
    ...
    sock->newSock();  // 写不写都无所谓
    sock->m_isConnected = true;   // 默认无论合适都是链接
    return sock;
}
...
Socket::ptr Socket::accept() {
    Socket::ptr cli_sock(new Socket(m_family, m_type, m_protocol));
